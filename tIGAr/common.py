...
class AbstractExtractionGenerator(object):
    ...
    def genericSetup(self):
        ...
        # added begins ------------------------------------
        # for a scalar
        self.VE1d = FiniteElement(self.extractionElement(),\
                                  self.mesh.ufl_cell(),\
                                  self.getDegree(0))
        self.V1d = FunctionSpace(self.mesh,self.VE1d)
        # added ends --------------------------------------
...
class ExtractedSpline(object):
    ...
    def genericSetup(self):
        ...
        self.VE_DG = FiniteElement("DG",\
                                       self.mesh.ufl_cell(),0)
        ...
        self.V_DG = FunctionSpace(self.mesh,self.VE_DG)
    ...
    def solveLinearSystem(self,MTAM,MTb,u):
        ...
        if(self.linearSolver == None):
            solve(MTAM,MTU,MTb)
            # ---------------------
            # solve(MTAM,MTU,MTb,"lu")
            # solve(MTAM,MTU,MTb,"cg","hypre_amg")
            # ---------------------
        else:
    ...
    def solveNonlinearVariationalProblem(self,residualForm,J,u,
        ...
        for i in range(0,self.maxIters):
            ...
            if(MPI.rank(self.comm) == 0):
                ...
                #       + str(relativeNorm)+", Abs norm: "+str(currentNorm))
                print("Solver iteration: "+str(i)+" , Relative norm: %e, Abs norm: %e" \
                    % (relativeNorm,currentNorm))
                # print("Current error:"+str(currentNorm))
                sys.stdout.flush()
            if(relativeNorm < self.relativeTolerance or currentNorm<1.e-6):
                converged = True
                break
            du = Function(self.V)
    ...
    # ------------------------------------------------------------------------
    # for 
    def solveNonlinearVariationalProblemLinearSpace(self,residualForm,J,u,
                                                    referenceError=None):
        """
        Solves a nonlinear variational problem with residual given by 
        ``residualForm``.  ``J`` is the functional derivative of 
        ...
        for i in range(0,self.maxIters):
            # MTAM,MTb = self.assembleLinearSystem(J,residualForm)
            # A,b = self.assembleLinearSystemLinearSpace(J,residualForm)
            A = assemble(J)
            b = assemble(residualForm)
            ...
            if(relativeNorm < self.relativeTolerance or currentNorm<1.e-6):
                converged = True
                break
            du = Function(self.V_linear)
            # #du.assign(Constant(0.0)*du)

            # self.solveLinearSystemLinearSpace(A,b,du)
            
            solve(A,du.vector(),b,'cg','hypre_amg')

            # #as_backend_type(u.vector()).vec().assemble()
            # #as_backend_type(du.vector()).vec().assemble()
            # # -----------------------
            # U = du.vector()
            # solve(A,U,b,'cg','default')
            # # if(self.linearSolver == None):
            # #     solve(A,U,b)
            # # else:
            # #     self.linearSolver.solve(A,U,b)
            
            # du.vector().set_local( U.get_local() )
            # du.vector().apply('')
            # # -------------------------
        ...
        if(not converged):
            print("ERROR: Nonlinear solver failed to converge.")
            exit()
    def assembleLinearSystemLinearSpace(self,lhsForm,rhsForm,applyBCs=False):
        """
        Assembles a linear system corresponding the LHS form ``lhsForm`` and
        RHS form ``rhsForm``.  The optional argument ``applyBCs`` is a 
        Boolean indicating whether or not to apply the spline's 
        ...
        # really an unnecessary function now that i split out the
        # vector and matrix assembly...
        return (self.assembleMatrixLinearSpace(lhsForm,applyBCs),
                self.assembleVectorLinearSpace(rhsForm,applyBCs))
    def assembleMatrixLinearSpace(self,form,applyBCs=False,diag=1):
        """
        Assemble M^T*A*M, where ``form`` is a bilinear form and the Boolean
        ``applyBCs`` indicates whether or not to apply the Dirichlet BCs.
        The optional argument ``diag`` is what will be filled into diagonal
        ...
        return MTAM
    def assembleVectorLinearSpace(self,form,applyBCs=False):
        """
        Assemble M^T*b, where ``form`` is a linear form and the Boolean
        ``applyBCs`` indicates whether or not to apply the Dirichlet BCs.
        """
        ...
        return MTb
    def solveLinearSystemLinearSpace(self,MTAM,MTb,u):
        """
        Solves a linear system of the form
        ...
        if(self.linearSolver == None):
            ...
            self.linearSolver.solve(MTAM,U,MTb)
        # u.vector().set_local(((self.M)*MTU).get_local())
        u.vector().set_local( U.get_local() )
        as_backend_type(u.vector()).vec().ghostUpdate()
        as_backend_type(u.vector()).vec().assemble()
    ...
    def projectScalarOntoLinears(self,toProject,linearSolver=None,
        ...
        if(lumpMass):
            ...
            # lhsForm = inner(Constant(1.0),v)*self.dx
        else:
            lhsForm = inner(u,v)*self.dx.meas #lhs(res)
            # lhsForm = inner(u,v)*self.dx #lhs(res)
        rhsForm = inner(toProject,v)*self.dx.meas #rhs(res)
        # rhsForm = inner(toProject,v)*self.dx #rhs(res)
        A = assemble(lhsForm)
        b = assemble(rhsForm)
        ...
        return u
    # --------------------------------------------------------------------------
    # to accelarate the speed only
    def projectScalarOntoLinearsUsingNonlinear(self,toProject,linearSolver=None):
        """
        L2 projection of some UFL object ``toProject`` onto a space of linear,
        scalar FE functions (typically used for plotting).  The optional 
        parameter ``lumpMass`` is a Boolean indicating whether or not to 
        ...
        ``None`` is passed.
        """
        du = TestFunction(self.V_linear)
        u = Function(self.V_linear)

        F = inner((u-toProject),du)*self.dx.meas
        J = derivative(F,u)
        bcs = []
        ...
        return u
    # ----------------------------------------------------
    # project a scalar onto DG 
    def projectScalarOntoDG(self,toProject,linearSolver=None,
                                 lumpMass=False):
        """
        L2 projection of some UFL object ``toProject`` onto a space of linear,
        scalar FE functions (typically used for plotting).  The optional 
    ...
    # --------------------------------------------------------------------------
    # to accelarate the speed only
    def solveNonlinearVariationalProblemDG(self,residualForm,J,u,
                                                    referenceError=None):
        """
        Solves a nonlinear variational problem with residual given by 
        ``residualForm``.  ``J`` is the functional derivative of 
